    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-black text-[#0A1628]">My Jobs</h1>
        ...
        {loading ? (
          <div className="flex flex-col gap-4">
            {[1, 2].map((i) => (
              <div key={i} className="bg-white border border-[#E5E7EB] rounded-2xl p-5 animate-pulse">
                ...
                <div className="h-3 bg-[#E5E7EB] rounded w-1/3 mb-4" />
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-black text-[#0A1628] mb-2">No jobs posted yet</h2>
          ...
          <div className="flex flex-col gap-4">
            {jobs.map((job) => (
              <div
                key={job.id}
                className="bg-white border border-[#E5E7EB] rounded-2xl p-5 hover:shadow-md hover:border-[#1E6FFF] transition-all cursor-pointer"
                onClick={() => router.push(`/jobs/${job.id}`)}
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <h3 className="font-black text-[#0A1628] truncate">{job.title}</h3>
                      <Badge variant={STATUS_BADGE[job.status]} size="sm">
                        {STATUS_LABELS[job.status]}
                      </Badge>
                    </div>
                    <span className="text-xs font-semibold text-[#1E6FFF] bg-[#EFF6FF] px-2 py-0.5 rounded-full">
                      {getCategoryIcon(job.category)} {getCategoryLabel(job.category)}
                    </span>
                  </div>
                  <ChevronRight size={18} className="text-[#9CA3AF] flex-shrink-0 mt-1" />
                </div>
